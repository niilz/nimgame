...
impl MoveCollector {
    ...
    fn collect_all_games(&mut self, moves: &[Condition], remaining_matches: u8) {
        ...
        match remaining_matches {
            // This Player should draw all but one and will win,
            // because next player has to draw 1
            2..=4 => {
                let second_to_last_move = Condition {
                    player: current_player,
                    drawn: remaining_matches - 1,
                    remaining: remaining_matches,
                };
                let mut finished_game_moves = moves.to_vec();
                finished_game_moves.push(second_to_last_move);
                let last_move = Condition {
                    player: next_player,
                    drawn: 1,
                    remaining: 1,
                };
                finished_game_moves.push(last_move);
                let finished_game = Game {
                    winner: current_player,
                    moves: finished_game_moves,
                };
